...
// ================= TokenToken  start =======================
contract ITLeadProToken is StandardToken, Pausable{
    string public constant name = "ITLeadPro_TOKEN";
    string public constant symbol = "ITLP";
    uint256 public constant decimals = 18;

    address public tokenSaleAddress;
    address public actionServiceAddress;
    address public tokenDepositAddress; // MultiSigWallet

    uint256 public constant tokenDeposit = 100000000 * 10 ** decimals;

    function ITLeadProToken(address _tokenDepositAddress) public {
        tokenDepositAddress = _tokenDepositAddress;

        balances[tokenDepositAddress] = tokenDeposit;
        Transfer(0x0, tokenDepositAddress, tokenDeposit);
        totalSupply_ = tokenDeposit;
    }
    ...
    function setTokenSaleAndActionServiceAddress(address _tokenSaleAddress,  address  _actionServiceAddress) public onlyOwner {
        if (_tokenSaleAddress != address(0)) {
            tokenSaleAddress = _tokenSaleAddress;
        }

        if (_actionServiceAddress != address(0)) {
            actionServiceAddress = _actionServiceAddress;
        }
    }
    ...
    function mint(address _recipient, uint256 _value) public whenNotPaused returns (bool success) {
        ...
        require(msg.sender == tokenSaleAddress||msg.sender == actionServiceAddress);

        balances[tokenDepositAddress] = balances[tokenDepositAddress].sub(_value);
        balances[_recipient] = balances[_recipient].add(_value);

        Transfer(tokenDepositAddress, _recipient, _value);
        return true;
...
contract TokenSale is Pausable {
    ...
    event RefundToken(address to, uint256 val);
    event LogToken(address add, uint256 val);

    function TokenSale(
        ITLeadProToken _tokenTokenAddress,
    ...
    function buy(address to, uint256 val) internal returns (bool success) {
        MintToken(tokenDepositAddress, to, val);
    ...
    function createTokens(address _beneficiary, uint256 _value) internal whenNotPaused {
        ...
        if (etherToRefund > 0){
            RefundToken(msg.sender, etherToRefund);
            msg.sender.transfer(etherToRefund);
        }
        tokenDepositAddress.transfer(this.balance);
        return;
    }
